...
class WindowsEmulator(BinaryEmulator):
    ...
    def init_module(self, modconf={}, name='none', emu_path='', default_base=None):
        ...
        # CP FIXME
        # This one was mem_reserve
        self.mem_map(size, base=res, tag='emu.module.%s' % (mod.name),
                         perms=common.PERM_MEM_RW)
        # CP FIXME
        if modname == 'ntdll' or modname == 'kernel32' or modname == 'kernelbase' or modname == 'advapi32':
            self.mem_write(res, mod.get_memory_mapped_image())
